...
local classProtectedAccessors = {}
local classPrivateAccessors = {}
...
-- Registers templates to be used during class registration
local registerTemplate = function(tab)
    validateTemplate(tab)
    classTemplates[tab.name] = tab
end
...
do
    ...
    new = function(temp)
        ...
        local inheritedClassName = temp.inherits
        if inheritedClassName then
            classInheriteds[temp.name] = inheritedClassName
            if not classes[inheritedClassName] then
                local inheritedTemp = classTemplates[inheritedClassName]
                new(inheritedTemp)
            end
            local inheritedClassDatas = {}
            local nextInheritedClassName = inheritedClassName
            while nextInheritedClassName do
                table.insert(inheritedClassDatas, 1, classDatas[nextInheritedClassName])
                nextInheritedClassName = classInheriteds[nextInheritedClassName]
            end
        ...
        classes[className] = staticProxy
    end
end

local registerClass = function(name)
    if type(name) ~= "string" then
        error(errors.badStr("name"))
    end
    local temp = classTemplates[name]
    if not temp then
        error("No class template with the name "..name.." exists")
    end
    new(temp)
end
...
qclass.registerTemplate = registerTemplate
qclass.registerClass = registerClass
qclass.classes = setmetatable({}, {
    __index = function(_, k)
        return classes[k]
    end,
    __newindex = function()
        error("Cannot set member of qclass.classes")
    end
})
